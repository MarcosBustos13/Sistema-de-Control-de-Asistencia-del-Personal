...
class VentanaSalida(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.title("Registrar Hora de Salida")
    ...
    def _crear_interfaz(self):
        ...
        ttk.Button(botones_frame, text="Registrar", command=self._registrar_salida).pack(side='left', padx=10)
    ...
    def _registrar_salida(self):
        """Registra la hora de salida en la base de datos."""
        ...
        try:
            ...
            if registro:
                # Si existe un registro, actualizar la hora de salida
                query = """
                UPDATE ASISTENCIA
                SET hora_salida = ?
                WHERE id = ?
                """
                self.conexion.ejecutar_consulta(query, (hora_salida, registro[0]))
                logging.info(f"Actualizando registro: hora_salida={hora_salida}, registro_id={registro[0]}")
            else:
                # Si no existe un registro, insertar uno nuevo con hora_entrada predeterminada
                query = """
                INSERT INTO ASISTENCIA (cedula_id, dia, fecha, hora_entrada, hora_salida, usuario, observaciones)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """
                dia = ahora.strftime("%A")
                self.conexion.ejecutar_consulta(query, (cedula, dia, fecha, '00:00:00', hora_salida, usuario_id, None))
                logging.info(f"Insertando nuevo registro: cedula_id={cedula}, dia={dia}, fecha={fecha}, hora_salida={hora_salida}, usuario_id={usuario_id}")

            # Confirmar la transacción
            self.connection.commit()
            logging.info("Commit realizado exitosamente.")
            messagebox.showinfo("Éxito", "Salida registrada exitosamente.")
            self._cerrar()
        except Exception as e:
            logging.error(f"Error al registrar la salida: {str(e)}", exc_info=True)
