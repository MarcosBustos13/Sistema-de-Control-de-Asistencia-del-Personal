...
class VentanaSalida(tk.Toplevel):
    ...
    def _crear_interfaz(self):
        ...
        ttk.Button(frame_botones, 
                 text="Registrar", 
                 command=self._registrar_salida).pack(side='left', padx=10)
    ...
    def _registrar_salida(self):
        ...
        if not cedula:
            messagebox.showwarning("Advertencia", "Ingrese la cédula.")
            return
        ...
        try:
            ...
            # 1. Verificar usuario y obtener tipo_trabajador
            ...
            if registro:
                if registro.hora_salida:
                    messagebox.showwarning("Advertencia", "Salida ya registrada.")
                    return
                cursor.execute("""
                    UPDATE ASISTENCIA 
                    SET hora_salida = ? 
                    WHERE id = ?
                """, (datetime.now().time(), registro[0]))
            else:
                cursor.execute("""
